...
<div class="form-card"><div class="form-card-body">
  <form method="POST" action="{{ route('admin.products.update',$product) }}" enctype="multipart/form-data">
        ...
        <div style="display:flex;gap:8px;align-items:flex-end;">
          <select name="category_id" id="categorySelect" class="form-control" required>
            @foreach($categories as $c)
              <option value="{{ $c->id }}" {{ old('category_id',$product->category_id)==$c->id?'selected':'' }}>{{ $c->icon }} {{ $c->name }}</option>
            @endforeach
          </select>
    ...
    <div class="form-group"><label>Deskripsi</label>
      <textarea name="description" class="form-control" rows="3" style="resize:vertical;">{{ old('description',$product->description) }}</textarea>
    </div>
    <div class="form-row">
      ...
      <div class="form-group"><label>Gambar Produk</label>
        ...
        @if($product->image)
          <div style="margin-top:10px;display:flex;align-items:center;gap:12px;">
            <img src="{{ asset('storage/'.$product->image) }}" style="width:84px;height:84px;object-fit:cover;border-radius:16px;border:1px solid rgba(231,224,213,.6);">
            <span style="font-size:13px;color:#57534e;">Gambar saat ini</span>
          </div>
        @endif
      </div>
      <div class="form-group"><label>Warna BG</label>
        <input type="color" name="bg_color" value="{{ old('bg_color',$product->bg_color) }}" class="color-picker">
      </div>
    </div>
    <div class="checkbox-group">
      <label class="checkbox-label">
        <input type="checkbox" name="is_available" value="1" {{ $product->is_available?'checked':'' }}> ✅ Tersedia
      </label>
      <label class="checkbox-label">
        <input type="checkbox" name="is_custom_order" value="1" {{ $product->is_custom_order?'checked':'' }}> ✨ Custom Order
      </label>
    </div>
